...
export default function DependentMiddlewareEditor({ nodeId }: { nodeId: string }) {
  const { id } = useParams();
  ...
  const addRow = () => {
    setRows([...rows, { id: Date.now().toString(), type: '', deployment: '单侧' }]);
  };
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="px-4 py-3 text-sm font-medium text-gray-700 w-1/2">中间件类型</th>
              <th className="px-4 py-3 text-sm font-medium text-gray-700 w-1/2">部署模型</th>
              ...
              <tr key={row.id} className="border-b border-gray-200 last:border-0 hover:bg-gray-50 group">
                ...
                <td className="px-4 py-3">
                  <input
                    ...
                    placeholder="输入或选择中间件类型"
                  />
                  <datalist id="middleware-types">
                    <option value="Redis" />
                    <option value="MySQL" />
                    <option value="Kafka" />
                ...
                <td className="px-4 py-3">
                  <select
                    ...
                    onChange={(e) => updateRow(row.id, 'deployment', e.target.value)}
                  ...
                  >
                    ...
                    <option value="双侧独立">双侧独立</option>
                    <option value="双侧bcp">双侧bcp</option>
                  </select>
                </td>
